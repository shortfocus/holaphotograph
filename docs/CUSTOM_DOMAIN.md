...
- [ ] **할 일**: api.holaphoto.com Access에서 **공개 경로 Bypass** 설정. 정책 선택기에는 URI Path/Request Method가 없음 → Zero Trust 응용 프로그램 편집 화면에서 **경로(Application path)** 설정이 있는지 확인하고, 있으면 공개 경로만 Bypass되도록 구성. 없으면 전체 Admin Only로 두고 공개 API 호출 구조(프록시 등) 검토.
...
#### api.holaphoto.com Access 화면별 상세 설정

**응용 프로그램 추가** 화면에서 아래 순서대로 진행합니다.

1. **기본 정보**
   - **응용 프로그램 이름**: `api.holaphoto.com` (그대로 사용)
   - **세션 기간**: 24시간 또는 원하는 값

2. **Access 정책 (순서 중요)**  
   정책은 **위에서부터** 먼저 매칭된 것이 적용됩니다. **1번 Bypass → 2번 Admin Only** 순서로 두세요.

   **정책 1 – 공개 API Bypass (로그인 없이 통과)**  
   - **기존 정책 선택**이 아니라 **새 정책 생성** 클릭  
   - **정책 이름**: `API 공개 경로 Bypass` (등 아무 이름)  
   - **동작(Action)**: **Bypass** 선택 (로그인 없이 통과)  
   - **규칙(Rules)** – 다음을 **Include**로 추가합니다.  
     (Cloudflare UI에 따라 “규칙 추가” 후 조건을 조합합니다.)
     - **필드**: `Request Method` → **같음** → `GET`  
     - 그리고 **다음 중 하나**를 만족하도록 규칙을 나누어 넣거나, 가능하면 **OR**로 묶습니다.  
       - `URI Path` **같음** `/`  
       - `URI Path` **같음** `/api`  
       - `URI Path` **시작** `/api/naver-rss`  
       - `URI Path` **시작** `/api/image-proxy`  
       - `URI Path` **시작** `/api/youtube-latest`  
       - `URI Path` **시작** `/api/channel-stats`  
       - `URI Path` **같음** `/api/reviews`  
       - `URI Path` **시작** `/api/posts` (GET만 있으므로 경로만으로 충분)  
       - `URI Path` **시작** `/api/images/`  
     - **POST**로 공개 허용할 경로가 있다면 별도 규칙으로:  
       - `Request Method` **같음** `POST` + `URI Path` **같음** `/api/lecture-signup`  
       - `Request Method` **같음** `POST` + `URI Path` **같음** `/api/reviews`  
       - `Request Method` **같음** `POST` + `URI Path` **같음** `/api/reviews/upload`  
   - 이 정책을 **목록의 1번(맨 위)**으로 두기

   **정책 2 – 관리자만 허용**  
   - **기존 정책 선택**에서 이미 쓰는 **Admin Only** 정책 선택  
   - (holaphoto.com/admin과 동일한 이메일/그룹 사용 권장)  
   - 이 정책을 **2번**으로 두기 (Bypass 다음)

   요약: **1번 = Bypass(공개 경로), 2번 = Allow(Admin Only)**. 나머지 관리자 전용 API는 2번에서 로그인 후 허용됩니다.

3. **로그인 방법**
   - **사용 가능한 모든 ID 공급자 수락** 또는 관리자만 쓸 **Google** 등 원하는 방식 선택  
   - 저장 후 **응용 프로그램 저장**으로 마침

이렇게 하면 메인 사이트(비로그인)는 공개 API만 호출하고, 관리자 전용 API는 Access 로그인 + Worker의 `ALLOWED_EMAILS` 검사로만 호출 가능합니다.
